...
import listTokenSei from "../_data/list-token-sei";
...
const TokenSelectEVM: React.FC<Props> = ({ value, onChange }) => {
  const [open, setOpen] = useState(false);

  const [input, setInput] = useState("");

  const listToken = useMemo(() => {
    return listTokenSei.filter(
      (element) =>
        element.symbol.toLowerCase().includes(input.toLowerCase()) ||
